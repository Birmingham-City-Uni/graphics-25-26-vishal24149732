...
int main()
{
	...
	for (int y = 0; y < (height/2); ++y)
		for (int x = 0; x < (width/2); ++x)
			for (int c = 0; c < 3; ++c) {
				int pixelIdx1 = x + y * width;
				int pixelIdx2 = (x + 1) + (y) * width;
				int pixelIdx3 = (x)+(y + 1) * width;
				int pixelIdx4 = (x + 1) + (y + 1) * width;
				float avg = (imageBuffer[pixelIdx1 * 4 + c] + imageBuffer[pixelIdx2 * 4 + c] +
					imageBuffer[pixelIdx3 * 4 + c] + imageBuffer[pixelIdx4 * 4 + c]) / 4.0f;
				imageBuffer[pixelIdx1 * 4 + c] = static_cast<uint8_t>(avg);
			}
