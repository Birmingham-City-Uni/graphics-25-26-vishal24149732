...
bool refract(const Vector3f& incident, const Vector3f& norm, float eta, Vector3f& refracted)
{
	...
	float k = 1 - (std::powf(eta, 2) * (1 - (std::powf(incident.dot(norm), 2))));
	if (k < 0) {
		return false;
	}

	refracted = (eta * incident) - (norm * (eta * (incident.dot(norm)) + std::sqrtf(k)));
...
Vector3f traceRay(const Ray& ray, const std::vector<Sphere>& spheres, const std::vector<std::unique_ptr<Light>>& lights, int bounce=0)
{
	...
	// and a coefft-wise product with the albedo.
		...
		Vector3f refracted;
		bool result = refract(ray.direction, normal, eta, refracted);
		if (result) {
			Ray refractedRay;
			refractedRay.origin = hitIntersection;
			refractedRay.direction = refracted;
			return traceRay(refractedRay, spheres, lights, bounce + 1);
		}
		else {
