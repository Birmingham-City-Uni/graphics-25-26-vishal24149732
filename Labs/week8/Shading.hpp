...
{
	// *** YOUR CODE HERE ***
	const  Eigen::Vector3f reflection = incoming + (2 * std::abs(incoming.dot(normal)) * normal.normalized());
	return reflection;
	// *** END YOUR CODE ***
}
...
float phongSpecularTerm(const Eigen::Vector3f& incomingLightDir, const Eigen::Vector3f& normal, const Eigen::Vector3f& viewDir, float exponent)
{
	...
	reflectionDir = reflect(incomingLightDir, normal);
	reflectionDir.normalized();
	viewDir.normalized();
	// Find dot product between reflected and view directions.

	float reflectDotNorm = 0.f;
	reflectDotNorm = reflectionDir.dot(viewDir);
	...
	return pow(reflectDotNorm , exponent) * (exponent + 2) / (2 * M_PI);
	// *** END YOUR CODE ***
}
...
float blinnPhongSpecularTerm(const Eigen::Vector3f& incomingLightDir, const Eigen::Vector3f& normal, const Eigen::Vector3f& viewDir, float exponent)
{
	...
	halfVec = (-incomingLightDir + viewDir).normalized();
	// Find dot product of half-vector and normal.
	float halfDotNorm = 0.f;
	halfVec.normalized();
	halfDotNorm = halfVec.dot(normal);
